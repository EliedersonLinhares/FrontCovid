...
const Cards = () =>{
    ...
    var deathsDay = DadosPIFullfetched.map(({ new_deaths }) => new_deaths);
    const calcDoubleTimeDeaths = () => Math.round(7/((deathsDay[1]/deathsDay[8])/7))
     
    var infectedDay = DadosPIFullfetched.map(({ new_confirmed }) => new_confirmed);
    const calcDoubleTimeInfected = () => Math.round(7/((infectedDay[1]/infectedDay[8])/7))

    var confirmeds = DadosPIFullfetched.map(({  last_available_confirmed}) =>  last_available_confirmed);
