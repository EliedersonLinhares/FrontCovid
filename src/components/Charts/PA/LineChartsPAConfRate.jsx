        ...
        ? (
            ...
            data={{
               // labels: DadosSPfetched.map(({ date }) => date).reverse(),
                labels: DadosPAFullfetched.map(({ date }) => date).reverse(),
                datasets: [
                  {
                    data: DadosPAFullfetched.map(({  last_available_confirmed_per_100k_inhabitants}) =>  (last_available_confirmed_per_100k_inhabitants).toFixed(2)).reverse(),
...
export default LineChartsPAConfRate;
/** 
 const LineChart =(
        DadosPA
        ? (
